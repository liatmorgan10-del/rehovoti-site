...
const BUSINESS_CATEGORIES = ["בית קפה", "מסעדה", "בר", "סטודיו", "חנות", "אחר"];
...
function CategoryIcon({ category, size = 16, color }) {
  const name = CATEGORY_ICONS[category] || "map-pin";
  return <i className={`ti ti-${name}`} style={{ fontSize: size, color }} aria-hidden="true"></i>;
}
...
const SITE_URL = "https://rehovoty.co.il";
...
function BusinessCard({ b }) {
  ...
  return (
    <a
      ...
      className="flex flex-shrink-0 items-center gap-3 rounded-full px-3.5 py-2"
      style={{ backgroundColor: "#F7F3EC", border: "0.5px solid #E5DFD0", fontFamily: "Rubik, sans-serif" }}
    >
      <img
        src={b.image_url}
        alt={b.name}
      ...
      <div className="min-w-0">
        ...
        <p className="truncate text-xs" style={{ color: COLORS.ink, opacity: 0.6 }}>
          {b.category} · {b.city}
        </p>
      </div>
      <i className="ti ti-brand-whatsapp flex-shrink-0" style={{ fontSize: 18, color: COLORS.ink }} aria-hidden="true"></i>
    </a>
  );
}
...
function InvitePill() {
  return (
    <a
      href="#footer-section"
      className="flex flex-shrink-0 items-center gap-2 rounded-full px-4 py-2"
      style={{ backgroundColor: "#F7F3EC", border: "0.5px dashed #C9BFA8", fontFamily: "Rubik, sans-serif" }}
    >
      <span className="text-sm font-medium" style={{ color: COLORS.ink }}>
        📣 כאן יכול להיות המקום שלכם! עסקים מקומיים - פנו אלינו ליצירת קשר לפרסום
      </span>
    </a>
  );
}

function BusinessMarquee({ businesses }) {
  const items = [];
  businesses.forEach((b, i) => {
    items.push(<BusinessCard key={b.id} b={b} />);
    if ((i + 1) % 3 === 0) items.push(<InvitePill key={`invite-${i}`} />);
  });
  items.push(<InvitePill key="invite-end" />);
  return (
    ...
    <div className="flex justify-center py-4">
      <div
        style={{
          ...
          height: size,
          borderRadius: "50%",
          border: `3px solid ${COLORS.inkLight}`,
...
function EventCard({ ev, isAdmin, onDelete, onEdit, onToggleFeatured }) {
  ...
  async function shareEvent() {
    ...
    const text = `${ev.title}\n📅 ${dateLabel}${ev.time ? ` בשעה ${ev.time}` : ""}\n📍 ${ev.city}${
      ev.venue ? ` · ${ev.venue}` : ""
    }\n\nעוד אירועים ופעילויות ב-רחובותי: ${SITE_URL}/?src=share`;
      ...
      if (ev.image_url) {
        try {
          ...
          if (navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({ title: ev.title, text, files: [file] });
            return;
          }
        } catch {
  ...
  return (
    <div
      className="event-card-anim flex h-full flex-col overflow-hidden rounded-2xl shadow-sm transition-transform duration-200 hover:-translate-y-0.5 active:scale-[0.98]"
    ...
    >
      <div className="relative h-32 w-full flex-shrink-0" style={{ backgroundColor: COLORS.inkLight }}>
        {isNewEvent(ev.created_at) && (
          <span
            className="absolute left-2 top-2 z-10 rounded-full px-2 py-0.5 text-xs font-semibold shadow-sm"
        ...
        >
          <CategoryIcon category={ev.category} size={40} color={COLORS.ink} />
        </div>
        <div
      ...
      <div className="flex flex-1 flex-col gap-2.5 p-4">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          >
            <CategoryIcon category={ev.category} size={13} color={COLORS.paper} /> {ev.category}
          </span>
          {ev.target_audience && (
            <span
        ...
        <h3
          ...
          style={{ color: COLORS.ink, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}
        >
          {ev.title}
        </h3>
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-1">
          ...
          <div className="flex items-center gap-2">
            {isAdmin && (
              <button
                onClick={() => onToggleFeatured(ev.id, ev.featured)}
                aria-label="הצגת מומלץ"
                className="flex items-center justify-center rounded-full p-2"
                style={{
                  backgroundColor: ev.featured ? COLORS.accent : COLORS.inkLight,
                  color: ev.featured ? COLORS.paper : COLORS.ink,
                }}
              >
            ...
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81a3 3 0 100-6 3 3 0 00-3 3c0 .24.04.47.09.7L8.04 9.81A2.99 2.99 0 006 9a3 3 0 000 6c.76 0 1.44-.3 1.96-.77l7.13 4.16c-.05.21-.08.43-.08.65a2.96 2.96 0 105.92 0 2.96 2.96 0 00-2.93-3z" />
              </svg>
            </button>
            {ev.vendor_url && (
              <a
...
function EventRow({ events, isAdmin, onDelete, onEdit, onToggleFeatured, emptyText }) {
  if (events.length === 0) {
    return (
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      {events.map((ev) => (
        <EventCard key={ev.id} ev={ev} isAdmin={isAdmin} onDelete={onDelete} onEdit={onEdit} onToggleFeatured={onToggleFeatured} />
      ))}
    </div>
  );
}
...
function DateGroupedEvents({ events, isAdmin, onDelete, onEdit, onToggleFeatured, emptyText }) {
  return (
    <div className="list-fade-anim">
      <DateGroupedEventsInner events={events} isAdmin={isAdmin} onDelete={onDelete} onEdit={onEdit} onToggleFeatured={onToggleFeatured} emptyText={emptyText} />
    </div>
  );
}

function DateGroupedEventsInner({ events, isAdmin, onDelete, onEdit, onToggleFeatured, emptyText }) {
  if (events.length === 0) {
    return (
      <p className="text-sm" style={{ color: COLORS.ink, opacity: 0.5, fontFamily: "Rubik, sans-serif" }}>
        {emptyText}
...
export default function App() {
  ...
  // Load businesses (featured listings), also live-updating via Realtime.
  useEffect(() => {
    let cancelled = false;
  ...
  // one row per day; the count is everyone who visited today. We also record
  // where they came from (via a ?src= link) so we can see what's working.
  useEffect(() => {
    ...
    async function countToday() {
      ...
      const source = params.get("src") || "ישיר";
      await supabase.from("daily_visits").upsert({ day: today, session_id: sessionId, source });
  ...
  async function handleImageUpload(e) {
    ...
    try {
      ...
      const fileName = `${Date.now()}-${baseName || "image"}.jpg`;
      const { data: uploadData, error } = await supabase.storage.from("event-images").upload(fileName, compressed, {
        contentType: "image/jpeg",
      });
      if (error) throw error;
      const { data } = supabase.storage.from("event-images").getPublicUrl(uploadData.path);
  ...
  async function handleBusinessImageUpload(e) {
    const file = e.target.files[0];
    if (!file) return;
    ...
    try {
      const compressed = await compressImage(file);
      const baseName = file.name.replace(/\.[^/.]+$/, "").replace(/[^a-zA-Z0-9\-_]/g, "");
      const fileName = `biz-${Date.now()}-${baseName || "image"}.jpg`;
      const { data: uploadData, error } = await supabase.storage.from("event-images").upload(fileName, compressed, {
        contentType: "image/jpeg",
      });
      if (error) throw error;
      const { data } = supabase.storage.from("event-images").getPublicUrl(uploadData.path);
      setBusinessForm((f) => ({ ...f, imageUrl: data.publicUrl }));
    } catch {
      setBusinessError("העלאת התמונה נכשלה, נסו שוב.");
    }
  ...
  async function handleAddBusiness(e) {
    e.preventDefault();
    if (!businessForm.name || !businessForm.phone || !businessForm.imageUrl) {
      setBusinessError("נא למלא שם, טלפון, ולהעלות תמונה/לוגו.");
      return;
    }
    setBusinessError("");
    const { error } = await supabase.from("businesses").insert({
      ...
      image_url: businessForm.imageUrl,
      website_url: businessForm.websiteUrl || null,
    });
    if (error) {
      setBusinessError("ההוספה נכשלה: " + error.message);
      return;
    }
    setBusinessForm({
      name: "",
  ...
  // Admin-only: load full visit history (all-time) and break it down by
  // source, by week, and by month. Re-runs whenever the panel is opened.
  // Fetches in pages of 1000 since that's Supabase's default per-request row cap.
  useEffect(() => {
    if (!isAdmin || !showStats) return;
    let cancelled = false;
    async function loadStats() {
      ...
      // Weekly: bucket by the Sunday that starts each row's week, last 8 weeks.
      const weekCounts = {};
      allRows.forEach((row) => {
  ...
  // Ticks every minute so events with a specific time disappear right when
  // they start, not just at midnight.
  ...
  const approvedEvents = useMemo(() => {
    ...
    return events.filter((ev) => {
      ...
      if (ev.date === todayStr() && ev.time) {
        const effectiveTime = ev.end_time || ev.time;
        const eventDateTime = new Date(`${ev.date}T${effectiveTime}`);
  ...
  // Admin-only: approved events that have already happened (hidden from the public).
  const pastApprovedEvents = useMemo(() => {
    const now = new Date(nowTick);
    return events
      .filter((ev) => {
        if (ev.status !== "approved") return false;
  ...
  const filtered = useMemo(() => {
    return approvedEvents
      ...
      .filter((ev) => (specificDate ? ev.date === specificDate : true))
      .filter((ev) => {
  ...
  }, [approvedEvents, query, category, city, dateFilter, specificDate]);

  const newEvents = useMemo(() => {
    return approvedEvents
      .filter((ev) => ev.created_at && Date.now() - new Date(ev.created_at).getTime() <= 24 * 60 * 60 * 1000)
  ...
  async function toggleFeatured(id, current) {
    ...
    if (!error) setEvents((prev) => prev.map((ev) => (ev.id === id ? { ...ev, featured: !current } : ev)));
  }
  ...
  function handleEditStart(ev) {
    ...
    setForm({
      ...
      targetAudience: ev.target_audience || TARGET_AUDIENCES[0],
      vendorName: ev.vendor_name || "",
      contactPhone: ev.contact_phone || "",
      vendorUrl: ev.vendor_url || "",
  ...
  async function handleSubmit(e) {
    ...
    const isDuplicate = events.some(
      (ev) =>
        ...
        ev.date === form.date &&
        ev.title.trim().toLowerCase() === form.title.trim().toLowerCase()
    );
    if (isDuplicate) {
      setSaveError("אירוע עם אותו שם ואותו תאריך כבר קיים באתר. אם זה עדכון לאירוע קיים, פנו אלינו דרך יצירת קשר.");
      return;
    }
    setSaveError("");
    const payload = {
    ...
    if (editingEventId) {
      ...
      if (error) {
        setSaveError("העדכון נכשל: " + error.message);
        return;
      }
      setEvents((prev) => prev.map((ev) => (ev.id === editingEventId ? { ...ev, ...payload } : ev)));
      setSubmitNotice("האירוע עודכן ✓");
      resetForm();
      setShowForm(false);
      return;
    }
  ...
  return (
    <div dir="rtl" style={{ backgroundColor: COLORS.pageBg, minHeight: "100vh" }}>
      <style>{`
        ...
        .event-card-anim { animation: cardFadeIn 0.35s ease-out both; }
        @keyframes listFadeIn { from { opacity: 0; } to { opacity: 1; } }
        .list-fade-anim { animation: listFadeIn 0.3s ease-out; }
        @keyframes menuSlideIn { from { opacity: 0; transform: translateY(-6px); } to { opacity: 1; transform: translateY(0); } }
        .menu-slide-anim { animation: menuSlideIn 0.2s ease-out; }
      `}</style>
      {/* top menu bar */}
      <div className="sticky top-0 z-10 flex justify-end px-4 py-2" style={{ backgroundColor: COLORS.pageBg }}>
        <div className="relative">
          ...
          {showMenu && (
            <div
              className="menu-slide-anim absolute left-0 mt-2 flex w-48 flex-col overflow-hidden rounded-xl shadow-lg"
              style={{ backgroundColor: COLORS.paper, fontFamily: "Rubik, sans-serif" }}
            >
              <button
                onClick={() => {
      ...
      <div className="mx-auto mb-5 max-w-3xl px-4 sm:px-6">
        <div className="flex gap-1.5 overflow-x-auto pb-1">
          ...
          >
            <i className="ti ti-layout-grid text-lg" aria-hidden="true"></i>
            הכל
          </button>
          {CATEGORIES.map((c) => (
            <button
            ...
            >
              <CategoryIcon category={c} size={19} color={category === c ? COLORS.paper : COLORS.ink} />
              {c}
            </button>
          ))}
        </div>
              ...
              <div className="relative sm:flex-1">
                {!specificDate && (
                  <span
                    ...
                    style={{ color: COLORS.ink, opacity: 0.5, fontFamily: "Rubik, sans-serif" }}
                  >
                    תאריך ספציפי
                  </span>
                )}
      ...
      <div id="publish-section" className="mx-auto mb-6 max-w-3xl px-4 sm:px-6">
        ...
        {editingEventId && showForm && (
          <p className="mt-2 text-sm font-semibold" style={{ color: COLORS.teal, fontFamily: "Rubik, sans-serif" }}>
            ✏️ עורכים אירוע קיים
          </p>
        )}
        {submitNotice && (
      ...
      {showForm && (
        <div className="mx-auto mb-8 max-w-3xl px-4 sm:px-6">
          ...
          >
            ...
            <p className="-mt-1 text-xs sm:col-span-2" style={{ color: COLORS.ink, opacity: 0.55 }}>
              לאירוע מתמשך (כמו שוק או מכירה) יש לציין שעת סיום - כל עוד היא לא הגיעה, האירוע ימשיך להופיע באתר.
            </p>
            <input
            ...
            />
            <p className="-mt-1 text-xs sm:col-span-2" style={{ color: COLORS.ink, opacity: 0.55 }}>
              רק קישור ישיר לתמונה (מסתיים ב-jpg/png וכו') - קישור לעמוד באינסטגרם/פייסבוק לא יעבוד. הכי פשוט: העלו ישירות מהטלפון בכפתור שמתחת.
            </p>
            <label
            ...
            <button
              ...
              style={{ backgroundColor: COLORS.accent, color: COLORS.paper }}
            >
              {editingEventId ? "עדכון האירוע" : "פרסום האירוע"}
            </button>
          </form>
        </div>
      )}
              ...
              {pendingEvents.map((ev) => (
                ...
                >
                  <p className="text-sm font-semibold" style={{ color: COLORS.ink }}>
                    <CategoryIcon category={ev.category} size={14} color={COLORS.ink} /> {ev.title}
                  </p>
                  <p className="text-xs" style={{ color: COLORS.ink, opacity: 0.65 }}>
      ...
      {!loading && (query || category !== "הכל" || city !== "הכל" || dateFilter !== "הכל" || specificDate) ? (
        ...
        <>
              ...
              <div className="flex gap-3 overflow-x-auto pb-2">
                {newEvents.map((ev) => (
                  <div key={ev.id} className="w-64 flex-shrink-0">
                    <EventCard ev={ev} isAdmin={isAdmin} onDelete={deleteEvent} onEdit={handleEditStart} onToggleFeatured={toggleFeatured} />
                  </div>
                ))}
              </div>
            </section>
          ...
          <section id="business-section" className="mx-auto mb-6 max-w-3xl px-4 sm:px-6">
            <div className="mb-2 flex items-center gap-2">
              <span
                className="rounded-full px-2.5 py-1 text-xs font-bold"
                style={{ backgroundColor: "#8FA888", color: "#173404" }}
              >
                🏪 עסקים ומקומות מומלצים
              </span>
            </div>
            <BusinessMarquee businesses={businesses} />
            {isAdmin && showBusinessForm && (
              <form
                onSubmit={handleAddBusiness}
                className="mt-3 grid grid-cols-1 gap-3 rounded-2xl p-4 sm:grid-cols-2"
                style={{ backgroundColor: COLORS.inkLight, fontFamily: "Rubik, sans-serif" }}
              >
                  {businessError && (
                    <div
                  ...
                  <label
                    ...
                    style={{ backgroundColor: COLORS.paper, color: COLORS.ink, border: `1px dashed ${COLORS.ink}44` }}
                  >
                    {uploadingBusinessImage ? "מעלה תמונה..." : "📷 העלו לוגו/תמונה (חובה)"}
                    <input type="file" accept="image/*" onChange={handleBusinessImageUpload} className="hidden" />
                  </label>
                  {businessForm.imageUrl && (
                    <img
                      src={businessForm.imageUrl}
                      alt="תצוגה מקדימה"
                      className="h-16 w-16 rounded-full object-cover sm:col-span-2"
                    />
                  )}
          ...
          <section className="mx-auto mb-6 max-w-3xl px-4 sm:px-6">
            ...
            {loading ? (
              ...
              <DateGroupedEvents
                key={`${category}-${city}-${dateFilter}-${query}`}
                events={filtered}
                isAdmin={isAdmin}
                onDelete={deleteEvent}
                onEdit={handleEditStart}
                onToggleFeatured={toggleFeatured}
                emptyText="אין עדיין אירועים באתר."
              />
            )}
          </section>
        </>
      ...
      <footer id="footer-section" className="mx-auto max-w-3xl px-4 pb-10 sm:px-6">
        <div className="flex flex-wrap items-center gap-4">
          ...
          {isAdmin && pastApprovedEvents.length > 0 && (
            <button
              onClick={() => setShowPastEvents((s) => !s)}
              className="text-sm underline"
              style={{ color: COLORS.ink, opacity: 0.6, fontFamily: "Rubik, sans-serif" }}
            >
              {showPastEvents ? "סגירת אירועים שעברו" : `אירועים שעברו (${pastApprovedEvents.length})`}
            </button>
          )}
          {isAdmin && (
            <button
              ...
              style={{ color: COLORS.ink, opacity: 0.6, fontFamily: "Rubik, sans-serif" }}
            >
              {showStats ? "סגירת נתוני מקור" : "מאיפה מגיעים? (30 יום)"}
            </button>
          )}
          {isAdmin ? (
            <button
        ...
        {showPastEvents && isAdmin && (
          <div className="mt-3 flex flex-col gap-3">
            {pastApprovedEvents.map((ev) => (
              <div
                key={ev.id}
                className="flex flex-col gap-2 rounded-xl p-3"
                style={{ backgroundColor: COLORS.paper, fontFamily: "Rubik, sans-serif" }}
        ...
        {showStats && isAdmin && (
          <div
            className="mt-3 flex flex-col gap-4 rounded-xl p-4"
            style={{ backgroundColor: COLORS.paper, fontFamily: "Rubik, sans-serif" }}
          >
            <div>
              <p className="mb-2 text-sm font-semibold" style={{ color: COLORS.ink }}>
                לפי מקור (מאז ומעולם - {sourceStats.reduce((sum, s) => sum + s.count, 0)} כניסות)
              </p>
            ...
            <div>
              <p className="mb-2 text-sm font-semibold" style={{ color: COLORS.ink }}>
                לפי שבוע (8 שבועות אחרונים)
              </p>
              <div className="flex flex-col gap-1.5">
                {weeklyStats.map(({ weekStart, count }) => (
                  <div key={weekStart} className="flex items-center justify-between gap-2">
                    <span className="text-sm" style={{ color: COLORS.ink }}>
                      שבוע של {new Date(weekStart + "T00:00:00").toLocaleDateString("he-IL")}
                    </span>
                    <span className="text-xs" style={{ color: COLORS.ink, opacity: 0.6 }}>
                      {count} כניסות
                    </span>
            ...
            <div>
              ...
              <div className="flex flex-col gap-1.5">
                {monthlyStats.map(({ month, count }) => (
                  <div key={month} className="flex items-center justify-between gap-2">
                    <span className="text-sm" style={{ color: COLORS.ink }}>
                      {new Date(month + "-01T00:00:00").toLocaleDateString("he-IL", { month: "long", year: "numeric" })}
                    </span>
                    <span className="text-xs" style={{ color: COLORS.ink, opacity: 0.6 }}>
                      {count} כניסות
                    </span>
            ...
            <p className="text-xs" style={{ color: COLORS.ink, opacity: 0.45 }}>
              כדי לתייג קישור: הוסיפו ?src=whatsapp או ?src=facebook לסוף כתובת האתר בקישור ששולחים.
            </p>
          </div>
        )}
        {showLogin && !isAdmin && (
