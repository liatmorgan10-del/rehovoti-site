...
// Trial-period notice shown near the top of the site.
// Set SHOW_TRIAL_NOTICE to false to hide it once the trial ends / pricing goes live.
const SHOW_TRIAL_NOTICE = true;
const TRIAL_NOTICE_TEXT =
  "האתר נמצא כרגע בתקופת הרצה. בתום תקופה זו יתווסף תשלום סימלי עבור פרסום מודעות באתר.";
...
function EventCard({ ev, isAdmin, onDelete, onSave, onApprove }) {
  ...
  async function saveEdit() {
    if (!draft.title || !draft.date || !draft.time || !draft.venue || !draft.city) {
      setEditError("נא למלא שם אירוע, תאריך, שעה, מקום ושכונה.");
      return;
    }
    setSaving(true);
    const ok = await onSave(ev.id, {
      ...
      vendor_url: draft.vendorUrl || null,
  ...
  if (editing) {
    const inputStyle = { backgroundColor: COLORS.pageBg, color: COLORS.ink };
    return (
      <div
        className="flex flex-col gap-2 rounded-2xl p-3 shadow-sm"
      ...
      >
        ...
        {editError && (
          <p className="rounded-lg px-2 py-1 text-xs" style={{ backgroundColor: COLORS.coral, color: COLORS.paper }}>
            {editError}
          </p>
        )}
        <input
        ...
        />
        <input
          placeholder="שם הספק / מארגן (יוצג באתר)"
        ...
        />
        <input
          placeholder="קישור לרכישה/הרשמה (אופציונלי)"
        ...
        <div className="flex gap-2 pt-1">
          <button
            ...
            style={{ backgroundColor: COLORS.teal, color: COLORS.paper, opacity: saving ? 0.6 : 1 }}
          >
            {saving ? "שומר..." : "שמירה"}
          </button>
          <button
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-3">
        ...
        {isAdmin && ev.agreed_to_terms && ev.agreed_to_terms_at && (
          <p className="text-xs" style={{ color: COLORS.ink, opacity: 0.55 }}>
            ✓ אישר/ה תקנון ב-
            {new Date(ev.agreed_to_terms_at).toLocaleDateString("he-IL", {
              day: "2-digit",
              month: "2-digit",
              year: "numeric",
            })}{" "}
            בשעה{" "}
            {new Date(ev.agreed_to_terms_at).toLocaleTimeString("he-IL", {
              hour: "2-digit",
              minute: "2-digit",
            })}
          </p>
...
function EventRow({ events, isAdmin, onDelete, onSave, emptyText }) {
  if (events.length === 0) {
    return (
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      {events.map((ev) => (
        <EventCard key={ev.id} ev={ev} isAdmin={isAdmin} onDelete={onDelete} onSave={onSave} />
      ))}
    </div>
  );
}
...
function DateGroupedEvents({ events, isAdmin, onDelete, onSave, emptyText }) {
  if (events.length === 0) {
    return (
      <p className="text-sm" style={{ color: COLORS.ink, opacity: 0.5, fontFamily: "Rubik, sans-serif" }}>
        {emptyText}
...
// Trial-period notice card. Position controlled from inside App()'s JSX below —
// move the <TrialNotice /> line to relocate it, no need to touch this component.
function TrialNotice() {
  if (!SHOW_TRIAL_NOTICE) return null;
  return (
    <div className="mx-auto mb-4 max-w-3xl px-4 sm:px-6">
      ...
      >
        ...
        >
          ...
          <circle cx="12" cy="16.5" r="0.5" fill={COLORS.accent} />
        </svg>
        <span>
          <b style={{ color: COLORS.accent }}>שימו לב:</b> {TRIAL_NOTICE_TEXT}
        </span>
      </div>
    </div>
  );
...
export default function App() {
  ...
  // Admin-only: save edits made to an existing event (any field). Returns true on success
  // so the calling component can exit edit mode, or false so it can show an error and stay open.
  async function updateEvent(id, patch) {
    const { error } = await supabase.from("events").update(patch).eq("id", id);
    if (error) {
      setSaveError("השמירה נכשלה: " + error.message);
      return false;
    }
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.title || !form.date || !form.time || !form.venue || !form.city) {
      setSaveError("נא למלא שם אירוע, תאריך, שעה, מקום ושכונה.");
      return;
    }
  ...
  return (
    <div dir="rtl" style={{ backgroundColor: COLORS.pageBg, minHeight: "100vh" }}>
      ...
      {/* trial-period notice — move this line to reposition */}
      <TrialNotice />
