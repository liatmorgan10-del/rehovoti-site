...
export default function App() {
  ...
  // Admin-only: load visitor source breakdown for the last 30 days.
  // Re-runs whenever the panel is opened, so it reflects the latest data.
  useEffect(() => {
    if (!isAdmin || !showStats) return;
    let cancelled = false;
    async function loadStats() {
      const since = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
      const { data } = await supabase.from("daily_visits").select("source, day").gte("day", since);
      ...
      <footer id="footer-section" className="mx-auto max-w-3xl px-4 pb-10 sm:px-6">
        ...
        {showStats && isAdmin && (
          ...
          >
            ...
            <p className="mt-1 text-xs" style={{ color: COLORS.ink, opacity: 0.45 }}>
