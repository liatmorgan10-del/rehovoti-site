...
function PromoMarquee() {
  ...
  return (
    <div className="overflow-hidden">
      <div className="promo-marquee flex gap-3">
        {item}
        {item}
        {item}
        {item}
      ...
      <style>{`
        .promo-marquee { width: max-content; animation: promo-scroll 18s linear infinite; }
...
function BusinessMarquee({ businesses }) {
  ...
  return (
    <div className="overflow-hidden">
      <div className="business-marquee flex gap-3">
        {[...businesses, ...businesses].map((b, i) => (
          <BusinessCard key={`${b.id}-${i}`} b={b} />
        ))}
      </div>
      <style>{`
        .business-marquee { width: max-content; animation: business-scroll 30s linear infinite; }
        @keyframes business-scroll { from { transform: translateX(0); } to { transform: translateX(-50%); } }
      `}</style>
    </div>
  );
}
...
function EventCard({ ev, isAdmin, onDelete, onEdit }) {
...
function EventRow({ events, isAdmin, onDelete, onEdit, emptyText }) {
  if (events.length === 0) {
    return (
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      {events.map((ev) => (
        <EventCard key={ev.id} ev={ev} isAdmin={isAdmin} onDelete={onDelete} onEdit={onEdit} />
      ))}
    </div>
  );
}
...
function DateGroupedEvents({ events, isAdmin, onDelete, onEdit, emptyText }) {
  return (
    <div className="list-fade-anim">
      <DateGroupedEventsInner events={events} isAdmin={isAdmin} onDelete={onDelete} onEdit={onEdit} emptyText={emptyText} />
    </div>
  );
}

function DateGroupedEventsInner({ events, isAdmin, onDelete, onEdit, emptyText }) {
  if (events.length === 0) {
    return (
      <p className="text-sm" style={{ color: COLORS.ink, opacity: 0.5, fontFamily: "Rubik, sans-serif" }}>
        {emptyText}
              ...
              <div className="flex gap-3 overflow-x-auto pb-2">
                {newEvents.map((ev) => (
                  <div key={ev.id} className="w-64 flex-shrink-0">
                    <EventCard ev={ev} isAdmin={isAdmin} onDelete={deleteEvent} onEdit={handleEditStart} />
                  </div>
                ))}
              </div>
            </section>
